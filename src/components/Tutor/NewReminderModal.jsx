    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={closeAndReset}>
      <View className="flex-1 justify-center bg-black/40 p-4">
        <View className="rounded-3xl bg-white p-4">
          <Text className="mb-4 text-lg font-bold text-slate-900">Novo lembrete</Text>

          <Text className="mb-1 text-xs text-slate-500">Tipo</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="mb-3 space-x-2">
            {TYPE_OPTIONS.map((t) => (
              <TouchableOpacity
                key={t.key}
              ...
              >
                <Text className="text-base">{t.emoji} {t.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
          ...
          <View className="mb-3 max-h-36">
            <ScrollView>
              {pets.map((p) => (
                <TouchableOpacity key={p.id} className="flex-row items-center space-x-3 py-2" onPress={() => setPetId(p.id)}>
                  <View className="h-9 w-9 items-center justify-center rounded-full" style={{ backgroundColor: p.color || '#EEE' }}>
                    <Text className="text-lg">{p.avatar || '🐾'}</Text>
                  </View>
                  <View>
                    <Text className="font-semibold text-slate-900">{p.name || p.petName}</Text>
                    <Text className="text-xs text-slate-500">{p.breed || p.species || ''}</Text>
                  </View>
                </TouchableOpacity>
              ))}
            </ScrollView>
          ...
          <Text className="mb-1 text-xs text-slate-500">Descrição</Text>
          <TextInput value={description} onChangeText={setDescription} placeholder="Ex: Reforço da V10" className="mb-3 rounded-2xl border border-slate-200 px-4 py-3 text-slate-900" placeholderTextColor="#94a3b8" />

          <Text className="mb-1 text-xs text-slate-500">Data</Text>
          <TouchableOpacity onPress={() => setShowDatePicker(true)} className="mb-3 justify-center rounded-2xl border border-slate-200 px-4 py-3">
            <Text>{date ? date.toLocaleDateString('pt-BR') : 'Selecionar data'}</Text>
          </TouchableOpacity>
          ...
          <View className="mt-4 flex-row justify-end space-x-3">
            <TouchableOpacity className="rounded-2xl px-4 py-3" onPress={closeAndReset}>
              <Text>Cancelar</Text>
            </TouchableOpacity>
            <TouchableOpacity
